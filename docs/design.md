...
decisions I made, and the ones I'm still unsure about. Written before/while
building, lightly edited after; where a prediction turned out wrong I've
left the prediction and noted what actually happened.
...
*generation is the bottleneck*, and on a 0.5B fp32 model it's ~11 s/step for
B=2 × G=4 × 256 tokens. That's slow enough to matter for iteration speed and
fast enough to finish a 300-step run in under an hour — a reasonable trade
for not renting anything.

## Why this task

Constraints I wanted the task to satisfy:

1. **Verifiable reward** — graded by code, not a learned reward model.
   This is the RLVR premise and the whole reason GRPO is practical at hobby
   scale.
2. **Rich enough to be interesting** — not "output 42". Multiple reward
   terms with different learning speeds so the curve tells a story.
...
The reward is the attack surface — it's the only thing the policy
...
- **Hallucination uses the source entity list**, not the gold todos — the
  model shouldn't be able to invent plausible-looking owners that happen to
  not be in gold.
...
- **token-F1 over embedding cosine** for todo matching. Cosine with a small
  ...
  cosine is behind a config flag if paraphrase turns out to matter.
...
two runtimes — if Tier A and Tier B ever disagree on a score, that's a bug,
...
the PPO clip never fires. This is fine — even standard GRPO recipes often
do ~1 epoch per batch — but it means the clip is aspirational. Making it
*do* something requires minibatch reuse (multiple epochs per batch), which
is the next experiment; it doubles gradient compute per rollout, which is
exactly the thing rollouts-rich/cheap GPUs amortize and mine doesn't.
...
2. Data generator — gold known by construction.
...
- **Does the KL coefficient β=0.04 matter at this scale?** KL stayed under
  ...
  removed — or it's precisely *why* nothing went wrong. An ablation would
  tell, and it's cheap (one 300-step run with β=0).
